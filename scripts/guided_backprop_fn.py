...
if __name__ == '__main__':
    ...
    def loss_fn(output):
        return K.sum(output * K.variable(np.array(targets)))

    for layer in layer_list[-1:]:
        guided_backprops = deo.guided_backprop(
            layer, imgs, mode='custom', loss_fn=loss_fn)
        ...
        for i in range(k):
            ...
            plt.imshow(guided_backprops[i][..., 0], 'gray')
            plt.title('Guided Backprop CT')
            plt.axis('off')

            plt.subplot(nrow, ncol, 2*k + i+1)
            plt.imshow(guided_backprops[i][..., 1], 'gray')
            plt.title('Guided Backprop PET')
            plt.axis('off')

            plt.subplot(nrow, ncol, 3*k + i+1)
            plt.imshow(imgs[i][..., 0], 'gray')
            plt.imshow(guided_backprops[i][..., 0], 'jet', alpha=0.3)
            plt.title('Guided Backprop CT overlay')
            plt.axis('off')

            plt.subplot(nrow, ncol, 4*k + i+1)
            plt.imshow(imgs[i][..., 0], 'gray')
            plt.imshow(guided_backprops[i][..., 1], 'jet', alpha=0.3)
            plt.title('Guided Backprop PET overlay')
            plt.axis('off')

        plt.suptitle('Guided Backprop True Positive at Layer {}'.format(layer))
        # plt.show()
        plt.savefig(
            '../../results/hn_vis/guided_backprop_tp_layer_{}'.format(layer))
...
for layer in layer_list[-1:]:
    ...
    plt.suptitle('Guided Backprop False Positive at Layer {}'.format(layer))
    # plt.show()
    plt.savefig(
        '../../results/hn_vis/guided_backprop_fp_layer_{}'.format(layer))
